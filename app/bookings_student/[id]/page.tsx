...
import StudentBookingDetailsContent from "../../components/bookings/student/StudentBookingDetailsContent";
...
export default function StudentBookingDetailsPage() {
  ...
  useEffect(() => {
    ...
    if (!isStudent) {
      ...
      router.replace("/bookings_student");
    }
  }, [bookingId, isAuthenticated, isStudent, language, router]);

  useEffect(() => {
    if (!bookingId || !isAuthenticated || !isStudent) return;
    getBookingById(bookingId, dispatch).catch((e) => {
      console.error("Failed to load booking:", e);
    });
  }, [bookingId, dispatch, isAuthenticated, isStudent]);
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <main className={`${isRTL ? "mr-64" : "ml-64"} mt-16`}>
          <StudentBookingDetailsContent booking={currentBooking} />
        </main>
