...
import { getDepartmentById } from "../../store/api/departmentApi";
...
export default function DepartmentCoursesContent() {
  ...
  useEffect(() => {
    if (!departmentId || !collegeId || !universityId) {
    ...
    const loadData = async () => {
      try {
        setIsLoading(true);
        // Load department
        const departmentData = await getDepartmentById(departmentId, dispatch);
        setDepartment(departmentData);

        // Load courses filtered by department and courseType = university
        ...
        router.push(
          `/explore/universities/${universityId}/colleges/${collegeId}/departments`
        );
      } finally {
        setIsLoading(false);
      }
    ...
    loadData();
  }, [departmentId, collegeId, universityId, dispatch, router, language]);
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          >
            {language === "ar" ? "الدورات" : "Courses"} - {getDepartmentName(department)}
          </h1>
          <p
            className={`${
