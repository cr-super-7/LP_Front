...
import { getProfessorReviews, createProfessorReview } from "../../store/api/reviewApi";
...
export default function ProfessorReviews({ professorId }: ProfessorReviewsProps) {
  ...
  const loadReviews = async () => {
    try {
      setIsLoading(true);
      const reviewsData = await getProfessorReviews(professorId, dispatch);
  ...
  const handleSubmitReview = async (rating: number) => {
    ...
    try {
      await createProfessorReview(professorId, { rate: rating }, dispatch);
      await loadReviews();
      setIsReviewModalOpen(false);
      toast.success(
        language === "ar" ? "تم إضافة التقييم بنجاح" : "Review added successfully"
      );
    } catch (error: unknown) {
      ...
      if (
        ...
        errorMessage =
          language === "ar"
            ? "لقد قيّمت هذا الأستاذ مسبقاً"
            : "You have already reviewed this professor";
        setIsReviewModalOpen(false);
      } else if (!apiErrorMessage) {
        errorMessage =
          language === "ar" ? "فشل إضافة التقييم" : "Failed to add review";
      } else {
