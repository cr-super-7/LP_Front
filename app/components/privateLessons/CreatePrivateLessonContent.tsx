...
import { ArrowLeft, Upload, ChevronDown, X, Plus, Trash2 } from "lucide-react";
import { useRouter, useSearchParams } from "next/navigation";
import { useAppDispatch, useAppSelector } from "../../store/hooks";
import { createPrivateLesson } from "../../store/api/privateLessonApi";
import { getDepartments } from "../../store/api/departmentApi";
import type { CreatePrivateLessonRequest, ScheduleItem } from "../../store/interface/privateLessonInterface";
import toast from "react-hot-toast";

export default function CreatePrivateLessonContent() {
  ...
  // Get lessonType from query parameter
  const lessonTypeParam = searchParams.get("lessonType") as "department" | "professional" | null;

  // Redirect if no lessonType is provided
  useEffect(() => {
    if (!lessonTypeParam || (lessonTypeParam !== "department" && lessonTypeParam !== "professional")) {
      toast.error(language === "ar" ? "يرجى اختيار نوع الدرس" : "Please select lesson type");
      router.push("/private-lessons/my-lessons");
    }
  }, [lessonTypeParam, router, language]);

  const [formData, setFormData] = useState({
    lessonType: lessonTypeParam || "department",
    instructorNameAr: "",
    instructorNameEn: "",
    locationUrl: "",
    jobTitleAr: "",
    ...
    instructorImagePreview: null as string | null,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createPrivateLesson(lessonData, dispatch);
      toast.success(language === "ar" ? "تم إنشاء الدرس الخاص بنجاح" : "Private lesson created successfully");
      router.push("/private-lessons/my-lessons");
    } catch (error) {
      console.error("Failed to create private lesson:", error);
    }
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            >
              {language === "ar" 
                ? `إنشاء درس خاص - ${formData.lessonType === "department" ? "تابع لقسم" : "مهني"}`
                : `Create Private Lesson - ${formData.lessonType === "department" ? "Department" : "Professional"}`}
            </h1>
          </div>
        </div>
