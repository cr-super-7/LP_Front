...
export default function PrivateLessonsProperties() {
  ...
  useEffect(() => {
    const loadAdvertisements = async () => {
      try {
        setIsLoading(true);
        // Get both privateLessons and all type advertisements
        const [privateLessonsAds, allAds] = await Promise.all([
          getAdvertisements("privateLessons", dispatch),
          getAdvertisements("all", dispatch),
        ]);
        // Combine and remove duplicates
        const combinedAds = [...privateLessonsAds, ...allAds];
        const uniqueAds = combinedAds.filter(
          (ad, index, self) => index === self.findIndex((a) => a._id === ad._id)
        );
        setAds(uniqueAds);
      } catch (error) {
        console.error("Failed to load private lessons advertisements:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <section className="mb-12">
      <div className="flex items-center justify-between mb-4">
        ...
        >
          {language === "ar" ? "إعلانات الدروس الخصوصية" : "Private Lessons Advertisements"}
        </h2>
      </div>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              >
                {language === "ar"
                  ? "لا توجد إعلانات دروس خصوصية حالياً"
                  : "No private lessons advertisements right now"}
              </p>
              <p
              ...
              >
                {language === "ar"
                  ? "تابعنا، سيتم إضافة إعلانات جديدة للدروس الخصوصية قريباً."
                  : "Stay tuned, new private lessons promotions will be added soon."}
              </p>
            </div>
          </motion.div>
        </motion.div>
